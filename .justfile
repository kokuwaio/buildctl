...
# Run linter.
@lint:
    docker run --rm --read-only --volume=$PWD:$PWD:ro --workdir=$PWD kokuwaio/just:1.44.0
...
@test:
    ...
    docker run --rm --volume=$PWD:$PWD:ro --workdir=$PWD --env=CI_WORKSPACE=$PWD --env=BUILDKIT_HOST kokuwaio/buildctl:dev
